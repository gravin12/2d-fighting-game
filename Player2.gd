...
func _physics_process(delta): # movement
	if health > 0:
		if Input.is_action_pressed("right2"): #move right
			velocity.x = SPEED
			direction = 1
			if is_on_floor():
		...
		if Input.is_action_pressed("left2"): #move left
			velocity.x = -SPEED
			direction = 0
			if is_on_floor():
		...
		if Input.is_action_just_pressed("jump2") and jump > 0: #jump
		...
		if Input.is_action_just_pressed("atack2"):
			if direction == 1 or Input.is_action_pressed("righ2t"):
